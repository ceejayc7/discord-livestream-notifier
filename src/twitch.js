...
class Twitch {
    ...
    updateStreams = () => {
        let flattenStreamsString = Helpers.addQueryParamToList('user_login', Helpers.getListOfStreams('twitch')).join('');
    ...
    reduceResponse = async (response) => {
        ...
        for(const stream of response.data) {
            let preview = _.get(stream, 'thumbnail_url') + `?t=${Math.round((new Date()).getTime() / 1000)}`;
